...
osascript -e 'tell application "System Preferences" to quit'; sudo -v
# Keep-alive: update existing `sudo` time stamp until `macos` code has finished
...
sudo scutil --set ComputerName "Example"
sudo scutil --set HostName "Example"
sudo scutil --set LocalHostName "Example"
sudo defaults write /Library/Preferences/SystemConfiguration/com.apple.smb.server NetBIOSName -string "Example"
...
# Set a custom wallpaper image. `DefaultDesktop.jpg` is already default symlink
...
# Don’t group windows by application in Mission Control (use old Exposé behavior)
...
for app in "Activity Monitor" "cfprefsd" "Dock" "Finder" "Mail" "SystemUIServer"; do
